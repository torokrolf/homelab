...
- [Docker Compose automatic update](#dockerupd)
...
.
├── .github/
│   └── workflows/          # Docker Compose auto-update workflows (e.g. update-authentik.yml)
├── terraform/
│   └── proxmox-deploy/     # Creating/cloning VMs on Proxmox
├── ansible/
...
│   │   ├── docker_compose_update/ # Generic role: updates any Docker Compose stack
...
## Docker Compose automatic update

For applications running on K3s, **ArgoCD** automatically syncs whenever a manifest changes on GitHub. For services running in **Docker Compose** outside of K3s (e.g. Authentik), this used to be manual work: Renovate would find a new image version, I'd merge it to GitHub, then SSH into the server, manually edit the compose file, and run `docker compose up -d`.

This is now automated:

1. **Renovate** notices that a new version is available for a Docker image and opens a PR in the repo.
...
5. The playbook calls the generic **`docker_compose_update`** role, passing service-specific variables (`docker_compose_update_path`, `docker_compose_update_template`).
6. The role:
   - ensures the stack directory exists on the target host,
   - renders the fresh `docker-compose.yml` from the Jinja2 template,
   - pulls the new image and recreates the container if needed, using `pull: always` + `recreate: auto`.
7. A **Gotify** notification is sent at the start and end of the process (success ✅ / failure ❌).

**Why is the role generic?** The `docker_compose_update` role contains no service-specific data — the target directory and template path are always passed in as variables by the calling playbook. So for every new Docker Compose–based service (Traefik, Vaultwarden, etc.) all it takes is a new, few-line playbook plus its matching workflow file; the role itself never needs to change.

| File | Role |
|---|---|
| `.github/workflows/update-<service>.yml` | Tells GitHub Actions **when** (push to main, on a given path change) and **where** (self-hosted runner) to run |
| `ansible/update-<service>.yml` | Service-specific playbook — calls the generic role with the right variables |
| `ansible/roles/docker_compose_update/` | Generic, service-agnostic logic: file deploy + compose pull/recreate |
